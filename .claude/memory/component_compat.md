...
| custom:button-card | Fill full width of a sections-view column | WORKS | Needs BOTH `grid_options: { columns: 'full', rows: 'auto' }` AND `extra_styles: ":host { width:100% !important; max-width:100% !important; min-width:0 !important; display:block !important; flex:1 1 100% !important }"`. grid_options alone gives the card a wider slot but button-card's adoptedStyleSheets `:host { max-width: fit-content }` keeps it content-width without the !important override. Used on Lights page room wrapper + row template. | 2026-04-21 |
| custom:button-card | Embed a nested Lovelace card | WORKS | `custom_fields.<name>: { card: { type: vertical-stack, cards: [...] } }`. The nested card renders inside the button-card's shadow root and preserves interactive tap_action bindings on its children. Used on Lights page to put a vertical-stack of header + rows inside the glass-morph room wrapper. | 2026-04-21 |
