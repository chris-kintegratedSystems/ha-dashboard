...
## 2026-04-23: `ha-card { min-height: 100% }` inside button-card `:host`
**Tried:** extra_styles declaring `:host { min-height: var(--kis-card-h) }` plus `ha-card { min-height: 100% }` to make the visible card fill the button-card host allocation.
**Failed on sections-stack children (locks):** CSS spec: a child's `min-height: 100%` resolves against the parent's **definite** height, not against a `min-height`-only parent. `:host` had `min-height: 83px` but `height: auto`. In the vertical sections stack (locks), no parent grid enforced a definite row height, so `:host` stayed auto-sized; `ha-card`'s `min-height: 100%` resolved to 0; `ha-card` shrank to content-min (55px / 46px). In the 2-col garage grid, the default `align-items: stretch` gave `:host` a definite row height, and `min-height: 100%` resolved correctly, so garage `ha-card` filled its host (83/69px). Diagnostic probe showed locks at 55.219px vs garages at 83.000px on Tab S9 — ~28px delta at the visible card edge that earlier QA missed because it measured the button-card host (always 83) not `ha-card`.
